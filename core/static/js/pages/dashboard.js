...
// 		const lexicalCategories = jsonData.output.Grammaire["Champs Lexicaux"];
// 		if (lexicalCategories) {
// 			const lexicalKeys = Object.keys(lexicalCategories);
// 			const lexicalTotals = lexicalKeys.map(category => lexicalCategories[category].Total);
...
// 		console.log(verbes_temps, verbes_modes, phraseTypesForme, phraseTypesStructure, phraseTypesComplexite, typesDeDiscours, registresLinguistiques, figuresDeStyle);
...
const renderLexicalFieldBarChart = (categories, totals) => {
	const options = {
		...
		colors: [COLORS.primary],
		xaxis: { categories: categories, position: 'bottom' },
		yaxis: { min: 0, labels: { formatter: function (val) { return Math.floor(val) } } },
		title: { text: 'Total des Champs Lexicaux', align: 'center' },
		plotOptions: { bar: { horizontal: true, borderRadius: 5 } },
		responsive: [{ breakpoint: 600, options: { chart: { height: 250 } } }]
	};
	new ApexCharts(document.querySelector("#lexicalBarChart"), options).render();
};
